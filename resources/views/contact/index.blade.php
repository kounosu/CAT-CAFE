            ...
            <form action="{{ route('contact') }}" method="post">
                ...
                <div class="mb-4">
                    <label for=name_kana class="block text-left p-1 my-1 font-medium">お名前（フリガナ）<span
                            class="text-white text-xs bg-yellow-400 mx-2 py-1 px-2">必須</span></label>
                    <input id="name_kana"
                        class="w-full p-4 text-xs leading-none bg-blueGray-50 rounded outline-none border" type="text"
                        placeholder="例）タナカタロウ" name="name_kana" value="{{ old('name_kana') }}">
                    @error('name_kana')
                ...
                <div class="mb-4">
                    ...
                    <textarea id="body"
                        ...
                        type="text" placeholder="ご自由にご記入ください" name="body">{{ old('body') }}</textarea>
                    @error('body')
                    <p class="text-red-400">{{ $message }}</p>
                    @enderror
                </div>
